...
# Companion to:
#   - paper spec   : ${WORK_ROOT:-./work}/equissl-workspace/data/ssl_tbd_rerun.md
...
# already exist as symlinks in ${WORK_ROOT:-./work}/phase2_staging/seg_* are still re-printed —
...
EMPTY_CKPT="${WORK_ROOT:-./work}/tmp/empty_ckpt.pth"
...
emit_seg() {
  ...
  local OUT_TAG="${WORK_ROOT:-./work}/phase2_staging/seg_${INIT}_${RPE}_s${SEED}"
...
emit_depth() {
  local INIT=$1 RPE=$2 SEED=$3
  local OUT_TAG="${WORK_ROOT:-./work}/phase2_staging/depth_${INIT}_${RPE}_s${SEED}"
  local RPE_F; RPE_F=$(rpe_flags "$RPE")
  local CKPT; CKPT=$(init_ckpt "$INIT")
  local S1="${OUT_TAG}_stage1"
  local S2="${OUT_TAG}_stage2"
