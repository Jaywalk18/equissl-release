...
export TMPDIR=${WORK_ROOT:-./work}/tmp
...
init_ckpt() {
  case "$1" in
    rand) echo "${WORK_ROOT:-./work}/tmp/empty_ckpt.pth" ;;
...
run_seed_seg() {
  ...
  local OUT="${WORK_ROOT:-./work}/phase2_staging/seg_${INIT}_${RPE}_s${SEED}"
...
run_seed_depth() {
  local INIT=$1 RPE=$2 SEED=$3 GPU=$4
  local S1="${WORK_ROOT:-./work}/phase2_staging/depth_${INIT}_${RPE}_s${SEED}_stage1"
  local S2="${WORK_ROOT:-./work}/phase2_staging/depth_${INIT}_${RPE}_s${SEED}_stage2"
  local RPE_F; RPE_F=$(rpe_flags "$RPE")
  local CKPT; CKPT=$(init_ckpt "$INIT")
